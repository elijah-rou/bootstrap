...
Begin with the actual outcome (`changed`, `no-change`, `blocked`, or `failed`) and its evidence. Use the examples below for changed work. For no-change, explain why no edit is needed; for blocked or failed work, name the decision or failure. Report each validation command as passed, failed, or not run with its reason. Only validated changed work can become a PR; other reports remain evidence artifacts.
