...
function derivedInstallationStatus(record) {
  const components = Object.values(record.components);
  if (record.components.core === 'ready' && components.length > 0 && components.every(status => status === 'ready')) return 'ready';
...
switch (command) {
  ...
  case 'ready': { const record = load(); const status=derivedInstallationStatus(record);if(status!=='ready')fail('Installation readiness requires core and every started component to be ready');record.status=status;save(record);break; }
  case 'component-begin': { if(args.length!==1||!/^[a-z][a-z-]*$/.test(args[0]))fail('component-begin NAME');const record=load(false);record.components[args[0]]='installing';record.status=derivedInstallationStatus(record);save(record);break; }
  case 'component-ready': { if(args.length!==1||!/^[a-z][a-z-]*$/.test(args[0]))fail('component-ready NAME');const record=load();record.components[args[0]]='ready';record.status=derivedInstallationStatus(record);save(record);break; }
  ...
  case 'enroll': { if (args.length !== 1) fail('enroll ROOT'); const target = validateTarget(args[0]); const stat=lstatSafe(target); if(stat?.isSymbolicLink()) fail(`Refusing symlink enrollment root: ${target}`); if(stat&&!stat.isDirectory()) fail(`Enrollment root is not a directory: ${target}`); const record = load(); if (!record.enrolledRoots.includes(target)) record.enrolledRoots.push(target);if(stat){record.enrolledRootIdentities ||= {};record.enrolledRootIdentities[target]=`${stat.dev}:${stat.ino}`;}save(record);break; }
